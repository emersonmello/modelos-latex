...
{Configuração de Redes de Computadores Linux} % Nome da disciplina
{Prof. Emerson Ribeiro de Mello} % Nome do professor
{\url{mello@ifsc.edu.br}} % E-mail do professor
{\textbf{Aula 01:} Shell Script} % Título da aula
{14/06/2020} % Data da aula
...
\begin{note}[Nota][\faCircleInfo][notecolor]
...
\lstinputlisting[style=bash,caption={Meu primeiro shell script},label={intro}]{codigos/intro.sh}

\begin{tip}
    Para executar, digite: \dest[green]{\texttt{bash nome-do-arquivo.sh}} ou deixe o arquivo com permissão para execução, com o comando \dest[green]{\texttt{chmod +x nome-do-arquivo.sh}} e executá-lo da seguinte forma: \destx[green]{\texttt{./nome-do-script.sh}}.
\end{tip}
...
\lstinputlisting[style=bash,caption={Exemplo de definição e uso de variáveis},label={variaveis}]{codigos/variaveis.sh}

\lstinputlisting[style=bash,caption={Usando variáveis em conjunto com o comando read},label={nome}]{codigos/nome.sh}



\subsubsection{Variáveis de ambiente}

As \textit{variáveis de ambiente} (veja \autoref{tab:variaveis}) são aquelas que afetam o comportamento do interpretador de comandos e do \textit{shell script}. É importante frisar que cada \texttt{processo} possui seu ambiente. Um \textit{script} só pode exportar tuas variáveis para os processos filhos. Um \textit{script} invocado através da linha de comando não pode exportar de volta uma variável para o ambiente da linha de comando.

\begin{table}[!htpb]
    \centering
    \caption{Algumas variáveis de ambiente}\label{tab:variaveis}
...
\begin{table}[!htpb]
    \centering
    \caption{Operadores relacionais e lógicos}\label{tabelaoperadores}
...
\lstinputlisting[style=bash,caption={Estrutura de decisão SE},label={se}]{codigos/se.sh}

\lstinputlisting[style=bash,caption={Usando operador lógico E},label={se2}]{codigos/se2.sh}

\subsubsection{Escolha\ldots caso\ldots}


\lstinputlisting[style=bash,caption={Estrutura de decisão ESCOLHA},label={caso}]{codigos/caso.sh}
...
\lstinputlisting[style=bash,caption={Estrutura de repetição ENQUANTO},label={enquanto}]{codigos/enquanto.sh}

\subsubsection{Para}


\lstinputlisting[style=bash,caption={Estrutura de repetição PARA},label={para}]{codigos/para.sh}
